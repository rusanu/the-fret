...
function withHigh(r: Region): Region[] {
  const hi: Region = {
    ...
    id: r.id + 'h',
    name: r.name + ' ↑',
    startFret: r.startFret == 12 ? 0 : (r.startFret + 12) % 24,
...
function caged(R: number): Region[] {
  return [
      { id: 'caged-e', shortLabel: 'E', name: 'E shape', group: 'caged', startFret: R,                  endFret: R + 2  },
      ...
      { id: 'caged-a', shortLabel: 'A', name: 'A shape', group: 'caged', startFret: R + 7,              endFret: R + 9  },
